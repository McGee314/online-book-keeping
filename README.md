...
| Decision | Rationale |
|----------|-----------|
| **UserContext (ThreadLocal)** | Prevents users from accessing each other's data. Services call `UserContext.getUserId()` instead of accepting userId as a parameter — eliminates the risk of a user passing someone else's ID. |
| **Soft Delete (MyBatis-Plus @TableLogic)** | Records are never truly deleted. The `deleted` flag is automatically appended to all queries, allowing data recovery and audit trails. |
| **DTO/VO Pattern** | API contracts (DTOs/VOs) are separated from DB entities. This prevents accidental exposure of sensitive fields (e.g., hashed passwords in User entity) and allows API shapes to evolve independently from the schema. |
| **Axios Interceptors (Frontend)** | JWT token is attached automatically to every request via the request interceptor. The response interceptor handles 401 globally (clears token, redirects to login) — no component-level error handling needed. |
| **Vite Dev Proxy** | During development, Vite proxies `/api/*` requests to `http://localhost:8080`, eliminating CORS issues in dev mode without needing browser extensions. |
